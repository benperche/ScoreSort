...
- **Printing goes through PDFKit, never a hand-rolled view.** `PDFDocument.printOperation(for:scalingMode:autoRotate:)` works and backs the Combine tab's Print (⌘P, `Logic/PrintLogic.swift`). `NSPrintOperation(view:)` with a custom page-drawing `NSView` — the original attempt — and AppleScript automation both fail reliably. Duplex is set via the `printSettings` dictionary key `com_apple_print_PrintSettings_PMDuplexing`; `PMSetDuplex` isn't in the public SDK. **Set `NSPrintInfo.orientation` from the document's own page size** — booklet sheets are landscape and otherwise land on portrait paper — and keep centring **on** (the fold must sit at the paper's centre; centring only translates) with `scalingMode: .pageScaleDownToFit` (1:1 clips at the printer's unprintable margin). `.pageScaleDownToFit` **ignores `NSPrintInfo.scalingFactor`**, so the print dialog's Scale box is inert by design; scaling is `CombineOutputOptions.bookletPageScale`, applied during imposition so it reaches Create PDF and Preview too.
