...
| `stampAttributedString(_:)` | What actually gets drawn: the decoded rich text, else the plain text in `stampBaseAttributes`. Alignment follows the stamp's position (a right-hand stamp is right-aligned) and is added over the whole range **without** clobbering per-run fonts/colours. |
