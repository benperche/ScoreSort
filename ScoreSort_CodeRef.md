...
to whoever is doing the printing. In every case the printer is unknown at export time, so a saved
file must be an ordinary, sensible booklet: readable on screen, page order already solved, leaving
whoever prints it to apply their own knowledge of their own machine. Pre-rotating it would be the
app being clever about hardware it knows nothing about, and would look like a bug to anyone who
...
- `ctx.clip(to: half)` happens first, in sheet coordinates, so an oversized page is trimmed at the half's edge and can never bleed across the fold onto the facing page.

Because it lives in the imposition rather than in `NSPrintInfo`, it reaches **Create PDF and Open in Preview as well as Print** — a saved booklet is already right. This is also why printing keeps `.pageScaleDownToFit`: that mode ignores `NSPrintInfo.scalingFactor` (verified — 100% and 103% render byte-identical), so the print dialog's Scale box is inert and the app-side control is the single source of truth. `.pageScaleNone` *does* honour `scalingFactor`, but at the cost of the fit protection that rescues an A3 booklet sent to A4 paper.
...
| `StampToken` / `resolvingStampTokens(_:date:format:)` | `{date}` and `{year}` placeholders, filled in inside `stampAttributedString` so the preview and the written PDF resolve identically. Replacements run **back to front** (so earlier ranges stay valid) via `replaceCharacters`, which inherits the replaced run's attributes — a bold `{date}` yields a bold date. Both the measurement and the drawing go through `stampAttributedString`, so the border box sizes to the resolved text. |
| `StampDateFormat` | `dayFirst` (14/03/2026, **the default**), `longForm`, `iso`, `monthFirst`. Formatting uses a fixed `en_AU` locale, not the system one — a US-ordered date on a copyright line is actively misleading. Stored app-wide in `UserDefaults` under `stampDateFormat`; `.preferred` reads it. |
...
- **Input** — PDFs **and images** (`pageableFileExtensions`), loaded via `pdfDocument(forFileAt:)` so a JPEG/PNG/HEIC becomes an A4 page exactly as it would in the Combine tab. A drop anywhere in the tab is accepted (`.onDrop` on the tab's outer container, not on the drop zone), routed through the shared `collectDroppedFileURLs` and `expandToFiles(_:extensions:)`, so **folders expand recursively**, name-sorted, exactly as the Combine tab and Renamer behave.
  - Output is always a PDF, so an image **can't** replace its original on disk: `outputProblem` blocks the batch (with an explanation) while any non-PDF is queued in `replaceOriginal` mode. The first file becomes the preview page. "Add Files…" allows folders too.
...
| `ImagePagesTests` | `pdfDocument(forFileAt:)` / `pdfPages(fromImageAt:)` — an image becomes one A4 page, image pages carry a `pageRef` (the preview and flattener draw blanks without one), an image document stamps, PDFs load unchanged, junk loads as nil, extension sets line up |
...
| `BookletPlacementTests` | **Content placement** — eight solid-grey pages are imposed and every half-sheet is read back by sampling a pixel, so a transposed pair or mirrored sheet fails even when counts and sizes are right. Also asserts a 5-page part's three padding slots come out blank, that scaling down leaves an even margin on both edges of a half (proving it scales about the half's centre, not the page origin), and that scaling up is clipped at the fold instead of painting over the facing page. Pages are matched against *measured* greys, since rendering applies a gamma shift. |
