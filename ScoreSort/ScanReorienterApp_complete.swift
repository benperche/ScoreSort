    ...
    static let all: [WelcomeTourPage] = [
        ...
        WelcomeTourPage(
            ...
            bodyParagraphs: [
                ...
                "**Advanced: Booklets:** Switch the output menu at the bottom left from *Single pages* to **Booklet** and every part is imposed as its own fold-and-staple booklet — two pages to a sheet side, in saddle-stitch order, padded to a whole folded sheet. Choose **double size** (A4 parts onto A3 paper) or **fit A4 landscape** (an A5 booklet that prints on any printer). macOS has no booklet option of its own, so the whole folder goes out in one print job instead of one document at a time.",
                "**Booklets: setting up your printer.** macOS won't let an app choose your two-sided setting, and printers differ in which edge they turn the paper on — so the first time you print booklets, choose **Set Up Two-Sided Printing…** from the output menu. It prints one numbered sheet; fold it, and if the pages read 1, 2, 3, 4 the right way up you're done. If they don't, say so and ScoreSort corrects the setting for you. You only do this once per printer, and it's remembered separately for each, so home and school can differ. One thing not to be alarmed by: the print dialog's preview may show every second side upside down. That's normal — it's how the sheet has to be sent so it comes out right once the printer turns the paper over. Always judge it by what actually comes out of the printer. If your printer has no two-sided mode at all, print the odd pages, put the stack back in, and print the even pages using **Paper Handling** in the print dialog.",
                "**Booklets: exporting a PDF to print later.** Which way to turn the paper depends on the printer, so ScoreSort only applies it when *it* is doing the printing. **Create PDF** and **Open in Preview** always give you a booklet that reads the right way up on screen — an ordinary file you can check, keep, email, or carry to whatever printer you end up at. Whoever prints it, wherever, needs only: print it **double-sided**, choose whichever two-sided setting brings the backs out the right way up, then fold and staple. The page order is already sorted.",
                "Published music rarely matches A4 exactly, so booklet mode has a **Scale** box beside the output menu: 100% fits each page to its half of the sheet, higher fills more of it, lower leaves more space around the music. It applies to Create PDF and Preview as well as printing, so a booklet you save is already the right size.",
