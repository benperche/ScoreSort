    ...
    static let all: [WelcomeTourPage] = [
        ...
        WelcomeTourPage(
            ...
            bodyParagraphs: [
                ...
                "**Booklets: setting up your printer.** macOS won't let an app choose your two-sided setting, so the first time you print booklets there are two things to check. In the print dialog, set **Double-sided** to *On*. Then, because printers differ in which edge they turn the paper on, use **Print Test Sheet…** in the output menu: it prints a single sheet, front and back. Fold it — if the two sides read the right way up, you're set. If the back is upside down, switch **Two-sided flip** in the same menu and try once more. You only ever do this once per printer. If your printer has no two-sided mode at all, print the odd pages, put the stack back in, and print the even pages using **Paper Handling** in the print dialog.",
